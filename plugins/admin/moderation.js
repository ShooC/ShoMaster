exports.run = {
   ...
   async: async (m, {
      ...
      if (!optionList.includes(option)) return client.sendList(m.chat, '', `🪐 *Current status* : [ ${setting[type] ? 'ON' : 'OFF'} ]`, '', 'Klik', [{
         rows
      }], m)
      ...
      setting[type] = status
      client.reply(m.chat, Func.texted('bold', `🪐 ${Func.ucword(command)} has been ${option == 'on' ? 'activated' : 'inactivated'} Sukses.`), m)
   },
   admin: true,
   group: true,
   cache: true,
