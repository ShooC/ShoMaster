exports.run = {
   ...
   async: async (m, {
      ...
      if (!args || !args[0]) return client.sendList(m.chat, '', ` *Current status* : [ ${system[type] ? 'ON' : 'OFF'} ]`, '', 'Klik disini', [{
         rows
      }], m)
      let option = args[0].toLowerCase()
      let optionList = ['on', 'off']
      if (!optionList.includes(option)) return client.sendList(m.chat, '', ` *Current status* : [ ${system[type] ? 'ON' : 'OFF'} ]`, '', 'Klik disini', [{
         rows
      }], m)
